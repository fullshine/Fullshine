...
export default function ClienteActions({ customer }: { customer: Customer }) {
  ...
  async function handleSave() {
    setLoading(true)
    await updateCustomer(customer.id, { full_name: name, phone, email: email || undefined })
    setLoading(false)
    setMode(null)
    router.refresh()
  }
      ...
      <tr className="bg-blue-50">
        <td className="px-4 py-3" colSpan={4}>
          <div className="flex flex-wrap gap-2 items-end">
            ...
            <button onClick={() => setMode(null)} className="text-sm px-3 py-1 rounded border border-gray-300 hover:bg-gray-100">
              Cancelar
            </button>
          </div>
        </td>
      ...
      <tr className="bg-red-50">
        <td className="px-4 py-3" colSpan={4}>
          <div className="flex items-center gap-3">
            <span className="text-sm text-red-700">¿Eliminar a <strong>{customer.full_name}</strong>? Esta acción no se puede deshacer.</span>
